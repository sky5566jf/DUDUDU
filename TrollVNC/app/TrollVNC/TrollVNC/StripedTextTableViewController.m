    ...
    if (self.pullToReload) {
        self.refreshControl = ({
            ...
            [refreshControl addTarget:self
                               action:@selector(reloadTextDataFromEntry:)
...
- (void)loadTextDataFromEntry {
    ...
    if (stringPart.length == 0) {
        ...
        if (self.rowSeparator) {
            ...
            rowTexts =
                [[stringPart componentsSeparatedByCharactersInSet:[NSCharacterSet newlineCharacterSet]] mutableCopy];
        }
        ...
        if (self.rowPrefixRegularExpression) {
            ...
            for (NSString *row in rowTexts) {
                if (![self.rowPrefixRegularExpression firstMatchInString:row
                    ...
                    if (mRow)
                        [mRowTexts
                            addObject:[mRow stringByTrimmingCharactersInSet:[NSCharacterSet
            ...
            if (mRow)
                [mRowTexts
                    addObject:[mRow stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceAndNewlineCharacterSet]]];
            rowTexts = mRowTexts;
        }
...
- (void)resetAutoReload {
    ...
    uintptr_t eventMask = DISPATCH_VNODE_DELETE | DISPATCH_VNODE_WRITE | DISPATCH_VNODE_EXTEND | DISPATCH_VNODE_ATTRIB |
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.section == 0) {
        ...
        NSMutableAttributedString *mRowText = [[NSMutableAttributedString alloc] initWithString:rowText
                                                                                     attributes:rowAttrs];
        if (searchContent) {
            NSRange searchRange = [rowText rangeOfString:searchContent
                                                 options:NSCaseInsensitiveSearch | NSDiacriticInsensitiveSearch
                                                   range:NSMakeRange(0, rowText.length)];
            if (searchRange.location != NSNotFound) {
                [mRowText addAttributes:@{
                    ...
                    NSBackgroundColorAttributeName : [UIColor colorWithRed:253.0 / 255.0
                                                                     green:247.0 / 255.0
                                                                      blue:148.0 / 255.0
                                                                     alpha:1.0],
                }
                ...
                [UIAction actionWithTitle:NSLocalizedStringFromTableInBundle(@"Copy", @"Localizable",
                               ...
                               identifier:nil
                                  handler:^(__kindof UIAction *_Nonnull action) {
                                      [[UIPasteboard generalPasteboard] setString:content];
                                      if ([self.delegate
                                              respondsToSelector:@selector(stripedTextTableViewRowDidCopy:withText:)]) {
                                          [self.delegate stripedTextTableViewRowDidCopy:self withText:content];
                                      }
                                  }],
            ];
