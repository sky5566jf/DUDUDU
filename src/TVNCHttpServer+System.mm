...
- (TVNCHttpResponse *)handleInput:(NSDictionary *)query body:(NSData *)body {
    ...
    // 文本输入统一走 daemon 自身 inputText（剪贴板 + Cmd+V 兜底，任何 App 中英文必到；
    // 历史 8184 App 内 AX 通道已移除：daemon 进程调 AX 会崩溃，且 .deb 本无 App 输入服务）。
    TVLog(@"HTTP Server: Input via daemon inputText");
...
- (TVNCHttpResponse *)handleEndpoints:(NSDictionary *)query {
    ...
    // 「应用管理」= 4.63 从 MatisuTrollStore(8588) 合入的无前缀端点；旧「安装 / 卸载」分类 4.62 已清空但保留占位
    NSArray<NSString *> *categoryOrder = @[@"截图 / 流媒体", @"文件", @"剪贴板", @"输入", @"状态 / 设备",
                                           @"系统控制", @"应用管理", @"安装 / 卸载", @"网络调试", @"群控", @"页面"];
