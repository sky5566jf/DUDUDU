...
CFIndex CARenderServerGetDirtyFrameCount(void *) __attribute__((weak_import));
void CARenderServerRenderDisplay(kern_return_t a, CFStringRef b, IOSurfaceRef surface, int x, int y) __attribute__((weak_import));
...
- (instancetype)init {
    ...
    int width, height;
    // _unjailedReferenceBoundsInPixels is a private API, available on iOS 13+
    CGSize screenSize;
    if (@available(iOS 13, *)) {
        screenSize = [[UIScreen mainScreen] _unjailedReferenceBoundsInPixels].size;
    } else {
        // Fallback to nativeBounds (pixel dimensions)
...
- (BOOL)renderDisplayToScreenSurface:(IOSurfaceRef)dstSurface {
    ...
    if (CARenderServerRenderDisplay != NULL) {
        CARenderServerRenderDisplay(0, CFSTR("LCD"), dstSurface, 0, 0);
    } else {
        TVLog(@"CARenderServerRenderDisplay not available in simulator");
    }
    ...
    dispatch_once(&onceToken, ^{
        @autoreleasepool {
            ...
            // IOSurfaceAccelerator is only available on iOS 14+
            if (@available(iOS 14, *)) {
    ...
    // CARenderServerGetDirtyFrameCount may not exist on iOS 13 (weak_import).
    // If unavailable, treat screen as always changed (can't detect dirty frames).
    if (CARenderServerGetDirtyFrameCount == NULL) {
        TVLog(@"CARenderServerGetDirtyFrameCount not available, assuming screen changed");
    } else {
    ...
    // CARenderServerRenderDisplay may not exist on iOS 13 (weak_import).
    if (CARenderServerRenderDisplay == NULL) {
        TVLog(@"CARenderServerRenderDisplay not available on this iOS version");
        return NO;
    }
    ...
    // Use IOSurfaceAccelerator if available (iOS 14+), otherwise fallback to direct copy
    if (@available(iOS 14, *)) {
        if (accelerator) {
            IOSurfaceAcceleratorTransferSurface(accelerator, srcSurface, dstSurface, NULL, NULL, NULL, NULL);
        } else {
            // Fallback: direct IOSurface copy (slower but works on iOS 13)
            IOSurfaceRef dstSurfaceRef = (IOSurfaceRef)dstSurface;
            IOSurfaceRef srcSurfaceRef = srcSurface;
            size_t width = MIN(IOSurfaceGetWidth(dstSurfaceRef), IOSurfaceGetWidth(srcSurfaceRef));
            size_t height = MIN(IOSurfaceGetHeight(dstSurfaceRef), IOSurfaceGetHeight(srcSurfaceRef));
            IOSurfaceLock(srcSurfaceRef, kIOSurfaceLockReadOnly, NULL);
            IOSurfaceLock(dstSurfaceRef, 0, NULL);
            void *srcBase = IOSurfaceGetBaseAddress(srcSurfaceRef);
            void *dstBase = IOSurfaceGetBaseAddress(dstSurfaceRef);
            size_t srcBytesPerRow = IOSurfaceGetBytesPerRow(srcSurfaceRef);
            size_t dstBytesPerRow = IOSurfaceGetBytesPerRow(dstSurfaceRef);
            for (size_t y = 0; y < height; y++) {
                memcpy((uint8_t *)dstBase + y * dstBytesPerRow,
                       (const uint8_t *)srcBase + y * srcBytesPerRow,
                       width * 4); // ARGB: 4 bytes per pixel
            }
            IOSurfaceUnlock(dstSurfaceRef, 0, NULL);
            IOSurfaceUnlock(srcSurfaceRef, kIOSurfaceLockReadOnly, NULL);
        }
    } else {
        // iOS 13: direct IOSurface copy
        IOSurfaceRef dstSurfaceRef = (IOSurfaceRef)dstSurface;
        IOSurfaceRef srcSurfaceRef = srcSurface;
        size_t width = MIN(IOSurfaceGetWidth(dstSurfaceRef), IOSurfaceGetWidth(srcSurfaceRef));
        size_t height = MIN(IOSurfaceGetHeight(dstSurfaceRef), IOSurfaceGetHeight(srcSurfaceRef));
