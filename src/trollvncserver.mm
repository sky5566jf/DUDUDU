...
// Attempt to trigger WiFi reconnect via MobileWiFi private API
static void triggerWiFiReconnect() {
    // Try MobileWiFi.framework private API (iOS 13+)
    Class WiFiManager = NSClassFromString(@"WiFiManager");
    if (WiFiManager) {
        id mgr = [WiFiManager performSelector:@selector(sharedInstance)];
        if (mgr && [mgr respondsToSelector:@selector(joinNewNetwork)]) {
            [mgr performSelector:@selector(joinNewNetwork)];
            TVLog(@"WiFi: triggered reconnect via WiFiManager");
            return;
        }
    }
    // Fallback: notify SpringBoard to refresh WiFi state
    notify_post("com.apple.wifi.WIFI_EVENT_RSSI_UPDATED");
    TVLog(@"WiFi: notified WiFi event (fallback)");
}
...
static void parseDaemonOptions(void) {
    ...
    NSString *prefsSource = @"none";
    if (!prefs) {
        NSBundle *resBundle = tvResourceBundle();
        NSString *presetPath = [resBundle pathForResource:@"Managed" ofType:@"plist"];
        TVLog(@"-daemon: tvResourceBundle=%@, Managed.plist path=%@", resBundle, presetPath);
        if (presetPath) {
            prefs = [NSDictionary dictionaryWithContentsOfFile:presetPath];
            if (prefs) prefsSource = @"Managed.plist";
        }
    }

    if (!prefs) {
        prefs = [[NSUserDefaults standardUserDefaults] persistentDomainForName:@"com.82flex.trollvnc"];
        if (prefs) prefsSource = @"NSUserDefaults";
    }

    TVLog(@"-daemon: prefs source=%@, keys=%@", prefsSource, prefs ? [prefs allKeys] : @"(nil)");
    ...
    if ([httpPortN isKindOfClass:[NSNumber class]] || [httpPortN isKindOfClass:[NSString class]]) {
        ...
        if (v == 0) {
            gHttpPort = 5801; // default: enable noVNC on 5801
            TVLog(@"-daemon: HTTP Port=0 -> using default 5801");
        ...
        // HttpPort key not present in prefs (fresh install or prefs cleared).
        // Root.plist default only applies to UI display, not to the daemon plist.
        // Force-enable HTTP on 5801 so noVNC works out of the box.
        gHttpPort = 5801;
        TVLog(@"-daemon: HttpPort key missing -> using default 5801");
    }
    ...
    // Auto-unlock after boot (no passcode needed; Home x2 to dismiss lock screen)
    NSNumber *autoUnlockN = [prefs objectForKey:@"AutoUnlockEnabled"];
    if ([autoUnlockN isKindOfClass:[NSNumber class]]) {
        gAutoUnlockEnabled = autoUnlockN.boolValue;
        TVLog(@"-daemon: AutoUnlockEnabled=%@", gAutoUnlockEnabled ? @"YES" : @"NO");
    }
}
...
extern "C" {
void *tvncGetFrontBuffer(void) { return gFrontBuffer; }
...
static void displayHook(rfbClientPtr cl) {
    (void)cl;
    // The framebuffer resize (free+swap) is performed on a dedicated serial dispatch
    // queue (see maybeResizeFramebufferForRotation), NOT on the send thread. Doing it
    // here would take gFbStateMutex while already holding cl->sendMutex (displayHook runs
    // inside rfbSendFramebufferUpdate), inverting the lock order against the capture thread
    // (gFbStateMutex -> sendMutex) and deadlocking the daemon — frozen frames show as a
    // black noVNC. The queue takes only gFbStateMutex (then rfbNewFramebuffer's internal
    // sendMutex), preserving a single consistent lock order across all threads.
...
// Dedicated serial queue for framebuffer resize (free+swap). Running it here (off both
// the capture thread and the libvncserver send thread) is what makes the resize deadlock-
// free: it never holds a client sendMutex when it needs gFbStateMutex, so the global lock
// order stays gFbStateMutex -> sendMutex everywhere (no AB/BA with the capture/send threads).
static dispatch_queue_t tvGetResizeQueue(void) {
    static dispatch_queue_t q = NULL;
    static dispatch_once_t once = 0;
    dispatch_once(&once, ^{
        q = dispatch_queue_create("com.matisu.vnc.resize", DISPATCH_QUEUE_SERIAL);
    });
    return q;
}
...
NS_INLINE void maybeResizeFramebufferForRotation(int rotQ) {
    ...
    //     the desired geometry (gPendingResize=YES) and dispatch the free+swap to the dedicated
    //     resize queue (tvGetResizeQueue). The queue takes only gFbStateMutex (then rfbNew-
    //     Framebuffer's internal sendMutex), so it never inverts the lock order against the
    //     capture thread — eliminating BOTH the v4.22 UAF and the v4.24 deadlock.
    //   * With NO client connected: there is no concurrent encode, so resizing directly here
    //     is safe and keeps the framebuffer correctly sized for the screenshot API even when
    //     nobody is viewing.
    ...
    if (gClientCount > 0) {
        // A VNC client is connected and actively encoding, so we must NOT resize here on
        // the capture thread. Defer the free+swap to the dedicated resize queue (which holds
        // no client sendMutex), avoiding both the v4.22 UAF (rfbNewFramebuffer frees the old
        // buffer under a client mid-encode) and the v4.24 deadlock (send-thread resize took
        // gFbStateMutex under sendMutex). Only dispatch once per geometry change.
        if (!gPendingResize) {
            gPendingResize = YES;
...
// Performed on the dedicated resize queue (tvGetResizeQueue) when a client is connected,
// or directly on the capture thread when no client is connected. Runs under gFbStateMutex
// (see TvResizeLockGuard below) so it can never race the capture thread writing gBackBuffer.
// rfbNewFramebuffer() internally locks each client's sendMutex before freeing the old front
// buffer, so no client is mid-encode during the free (prevents the v4.22 UAF). Because the
// queue holds no client sendMutex when it takes gFbStateMutex, the global lock order stays
// gFbStateMutex -> sendMutex on every thread, so there is no AB/BA deadlock (the v4.24
// regression). See maybeResizeFramebufferForRotation() above.
static void performResizeFramebufferForRotation(void) {
    ...
    // Serialize the free+swap against the capture thread's render-into-back-buffer
    // (handleFramebuffer, which holds gFbStateMutex across its work). This is the ONLY
    // lock we take here. rfbNewFramebuffer() below internally locks each client's
    // sendMutex (under this lock) so no client is mid-encode while the old front buffer
    // is freed — that is what prevents the v4.22 UAF heap corruption. The global lock
    // order stays gFbStateMutex -> sendMutex on every thread, so there is no AB/BA
    // deadlock (the v4.24 regression that froze noVNC black).
    struct TvResizeLockGuard {
        TvResizeLockGuard() { pthread_mutex_lock(&gFbStateMutex); }
        ~TvResizeLockGuard() { pthread_mutex_unlock(&gFbStateMutex); }
    } _tvResizeLock;
    ...
    if (gScreen) {
        // Update server with new framebuffer (this frees the old front buffer under the
        // client sendMutex locks taken inside rfbNewFramebuffer, so no client is mid-encode).
    ...
    gPendingResize = NO; // resize applied; allow a future geometry change to dispatch again
    TVLog(@"Resize: framebuffer now %dx%d", gWidth, gHeight);
}
...
static void ptrAddEvent(int buttonMask, int x, int y, rfbClientPtr cl) {
    ...
    // Left button (bit 0) — 加同步保护，防止与群控 executeGroupTouchEvent 并发访问
    ...
    // 群控广播：主控模式下，将触摸事件广播给所有从控
    TVNCHttpServer *httpServer = [TVNCHttpServer sharedServer];
    if (httpServer.groupMasterEnabled && httpServer.groupSlaveCount > 0) {
        // 使用缓存的屏幕尺寸，避免每次 ptrAddEvent 都读 UIScreen（VNC 线程读主线程属性有风险）
        static CGFloat sCachedPixelW = 0, sCachedPixelH = 0;
        static UIInterfaceOrientation sCachedOrient = UIInterfaceOrientationUnknown;
        UIInterfaceOrientation curOrient = [UIApplication sharedApplication].statusBarOrientation;
        if (sCachedPixelW <= 0 || curOrient != sCachedOrient) {
            CGFloat screenScale = [[UIScreen mainScreen] scale];
            sCachedPixelW = [UIScreen mainScreen].bounds.size.width * screenScale;
            sCachedPixelH = [UIScreen mainScreen].bounds.size.height * screenScale;
            sCachedOrient = curOrient;
        }
        double nx = (sCachedPixelW > 0) ? pt.x / sCachedPixelW : 0;
        double ny = (sCachedPixelH > 0) ? pt.y / sCachedPixelH : 0;
        ...
        // 防回环：如果是群控注入的事件（电脑操控或中继转发），不上报
        if (!httpServer.isInjectingTouchEvent) {
            // 中继模式优先：通过电脑中转服务器异步广播（性能更好，支持100+设备）
            if (httpServer.relayModeEnabled && httpServer.isRelayConnected) {
                if (action) {
                    [httpServer reportRealTouchToRelay:action nx:nx ny:ny];
                }
            } else {
                // 传统直连模式：手机直接串行广播
                if (action) {
                    NSString *json = [NSString stringWithFormat:@"{\"type\":\"touch\",\"action\":\"%@\",\"x\":%.4f,\"y\":%.4f}", action, nx, ny];
                    [httpServer broadcastGroupEvent:json];
                }
            }
            ...
            if (midNow && !midPrev) {
                if (httpServer.relayModeEnabled && httpServer.isRelayConnected) {
                    NSDictionary *evt = @{@"type":@"key", @"action":@"power_down", @"source":@"real_touch"};
                    NSData *d = [NSJSONSerialization dataWithJSONObject:evt options:0 error:nil];
                    NSString *s = [[NSString alloc] initWithData:d encoding:NSUTF8StringEncoding];
                    [httpServer sendRelayMessage:s];
                } else {
                    [httpServer broadcastGroupEvent:@"{\"type\":\"key\",\"action\":\"power_down\"}"];
                }
            } else if (!midNow && midPrev) {
                if (httpServer.relayModeEnabled && httpServer.isRelayConnected) {
                    NSDictionary *evt = @{@"type":@"key", @"action":@"power_up", @"source":@"real_touch"};
                    NSData *d = [NSJSONSerialization dataWithJSONObject:evt options:0 error:nil];
                    NSString *s = [[NSString alloc] initWithData:d encoding:NSUTF8StringEncoding];
                    [httpServer sendRelayMessage:s];
                } else {
                    [httpServer broadcastGroupEvent:@"{\"type\":\"key\",\"action\":\"power_up\"}"];
                }
            }
            if (rightNow && !rightPrev) {
                if (httpServer.relayModeEnabled && httpServer.isRelayConnected) {
                    NSDictionary *evt = @{@"type":@"key", @"action":@"home_down", @"source":@"real_touch"};
                    NSData *d = [NSJSONSerialization dataWithJSONObject:evt options:0 error:nil];
                    NSString *s = [[NSString alloc] initWithData:d encoding:NSUTF8StringEncoding];
                    [httpServer sendRelayMessage:s];
                } else {
                    [httpServer broadcastGroupEvent:@"{\"type\":\"key\",\"action\":\"home_down\"}"];
                }
            } else if (!rightNow && rightPrev) {
                if (httpServer.relayModeEnabled && httpServer.isRelayConnected) {
                    NSDictionary *evt = @{@"type":@"key", @"action":@"home_up", @"source":@"real_touch"};
                    NSData *d = [NSJSONSerialization dataWithJSONObject:evt options:0 error:nil];
                    NSString *s = [[NSString alloc] initWithData:d encoding:NSUTF8StringEncoding];
                    [httpServer sendRelayMessage:s];
                } else {
                    [httpServer broadcastGroupEvent:@"{\"type\":\"key\",\"action\":\"home_up\"}"];
                }
            }
        }
    }
...
static UIInterfaceOrientation tvnc_currentInterfaceOrientation(void) {
    ...
    if (gSBGetActiveInterfaceOrientation) {
        r = gSBGetActiveInterfaceOrientation() & 0xFF;
        ...
        r = gSBGetInterfaceOrientation() & 0xFF;
    }
...
static void setupOrientationObserver(void) {
    ...
    // FBSOrientationObserver is only available on iOS 14+
    if (@available(iOS 14, *)) {
...
static void setupRfbScreen(int argc, const char *argv[]) {
    ...
    // ---------------------------------------------------------------------------
    // iOS 13.3.1 compatibility: ___darwin_check_fd_set_overflow
    //
    // ROOT CAUSE (found in v3.17): libvncserver.a was compiled with Xcode 11.4+
    // (SDK 13.4+). The FD_SET macro in that SDK emits a call to
    // ___darwin_check_fd_set_overflow(), which was introduced in iOS 13.4.
    // On iOS 13.3.1, this symbol does NOT exist in libSystem.B.dylib.
    // When rfbInitSockets calls FD_SET for the first time, dyld_stub_binder
    // fails to find the symbol and aborts with SIGABRT.
    //
    // FIX: We provide a weak implementation of ___darwin_check_fd_set_overflow
    // at the top of this file. Since libvncserver.a is statically linked into
    // the same Mach-O as our code, dyld will find our weak export when resolving
    // the lazy symbol pointer. On iOS 13.4+, the system's strong symbol in
    // libSystem.B.dylib takes precedence over our weak one.
    //
    // ADDITIONAL PROTECTION: On iOS < 13.6, we also pre-call network symbols
    // (getaddrinfo, socket, bind, etc.) and disable IPv6 listening. This is
    // not strictly necessary now that the weak symbol fix is in place, but
    // provides defense-in-depth in case there are other missing symbols we
    // haven't identified.
    // ---------------------------------------------------------------------------
    BOOL shouldDisableVncIPv6 = NO;
    if (@available(iOS 13.6, *)) {
        // iOS 13.6+ — all symbols work fine, keep IPv6
        shouldDisableVncIPv6 = NO;
    } else {
        // iOS < 13.6 — pre-bind network symbols as defense-in-depth.
        // The primary fix is the weak ___darwin_check_fd_set_overflow symbol
        // above, but we also pre-call these to ensure dyld has cached them.
        TVLog(@"iOS < 13.6 detected: pre-binding network symbols (defense-in-depth)");
        ...
        int gai_rv = getaddrinfo(NULL, "0", &hints, &res);
        if (gai_rv == 0 && res) {
            freeaddrinfo(res);
            TVLog(@"  getaddrinfo/freeaddrinfo: pre-bound OK");
        } else {
            TVLog(@"  getaddrinfo returned %d: %s", gai_rv, gai_strerror(gai_rv));
        }

        // 2. socket + bind + listen + setsockopt (IPv4 & IPv6 listen path)
        ...
        if (testSock >= 0) {
            ...
            sa.sin_port = 0; // let OS pick a port
            int opt = 1;
            setsockopt(testSock, SOL_SOCKET, SO_REUSEADDR, &opt, sizeof(opt));
            bind(testSock, (struct sockaddr *)&sa, sizeof(sa));
            listen(testSock, 1);
            close(testSock);
            TVLog(@"  socket/bind/listen/setsockopt: pre-bound OK");
        } else {
            TVLog(@"  socket() failed: errno=%d", errno);
        }

        // 3. getnameinfo + getpeername (peer address resolution path)
        ...
        getnameinfo((struct sockaddr *)&peerAddr, sizeof(peerAddr),
                    hostBuf, sizeof(hostBuf), servBuf, sizeof(servBuf), 0);
        // Ignore result; just force dyld binding

        int dummySock = socket(AF_INET, SOCK_STREAM, 0);
        if (dummySock >= 0) {
            struct sockaddr peer;
            socklen_t peerLen = sizeof(peer);
            getpeername(dummySock, &peer, &peerLen); // will fail, but binds symbol
            close(dummySock);
        }

        // 4. connect (client connection path)
        int connSock = socket(AF_INET, SOCK_STREAM, 0);
        if (connSock >= 0) {
            struct sockaddr_in connTarget;
            memset(&connTarget, 0, sizeof(connTarget));
            connTarget.sin_family = AF_INET;
            connTarget.sin_addr.s_addr = htonl(INADDR_LOOPBACK);
            connTarget.sin_port = htons(1); // port 1, will fail
            connect(connSock, (struct sockaddr *)&connTarget, sizeof(connTarget));
            close(connSock);
        }
...
static void setupRfbHttpServer(void) {
    ...
    TVLog(@"setupRfbHttpServer: gHttpPort=%d, gHttpDirOverride=%s, THEBOOTSTRAP=%d",
          gHttpPort, gHttpDirOverride ? gHttpDirOverride : "(null)",
#ifdef THEBOOTSTRAP
          1
#else
          0
    ...
    if (gHttpPort > 0) {
        ...
        gScreen->http6Port = gHttpPort; // same as v3.8: unconditional
        TVLog(@"setupRfbHttpServer: httpPort=%d, http6Port=%d", gScreen->httpPort, gScreen->http6Port);
        ...
        if (gHttpDirOverride) {
            ...
            // Compute httpDir relative to executable
            do {
                NSString *exe = tvExecutablePath();
                NSString *exeDir = [exe stringByDeletingLastPathComponent];
                NSString *webRel;
                ...
                TVLog(@"setupRfbHttpServer: exePath=%@, exeDir=%@, webRel=%@, webPath=%@, fs=%s, dirExists=%d",
                      exe, exeDir, webRel, webPath, fs ? fs : "(null)",
                      [[NSFileManager defaultManager] fileExistsAtPath:webPath] ? 1 : 0);
                if (fs && *fs) {
                    gScreen->httpDir = strdup(fs);
                    TVLog(@"HTTP server config: port=%d, dir=%@, proxyConnect=YES", gHttpPort, webPath);
                } else {
                    TVLog(@"setupRfbHttpServer: ERROR - fileSystemRepresentation failed for webPath=%@", webPath);
                }
            } while (0);
        }
    } else {
        ...
        TVLog(@"setupRfbHttpServer: HTTP disabled (gHttpPort=0)");
    }
...
static void initializeAndRunRfbServer(void) {
    TVLog(@"initializeAndRunRfbServer: BEFORE rfbInitServer - port=%d, ipv6port=%d, httpPort=%d, http6Port=%d, httpDir=%s",
          ...
          gScreen->httpDir ? gScreen->httpDir : "(null)");
...
static void cleanupAndExit(int code) {
    ...
    [[TVNCHttpServer sharedServer] stopServer];
...
// v3.43: 获取真实设备名（增强版，iOS 16+ daemon 上下文修复）
...
static void tvncCrashHandleSignal(int sig) {
    ...
    [log appendFormat:@"=== TrollVNC Server Crash Log ===\n"];
...
static void tvncSetupCrashLogger(void) {
    ...
    NSLog(@"[TrollVNC] Server crash logger initialized.");
}
...
int main(int argc, const char *argv[]) {
    ...
    /* Setup crash logger early */
    tvncSetupCrashLogger();
    ...
    @autoreleasepool {
        ...
        setupRfbHttpServer();
        [[TVNCHttpServer sharedServer] startServer];
        ...
        if (gIsDaemonMode && gAutoUnlockEnabled) {
            dispatch_async(dispatch_get_global_queue(QOS_CLASS_UTILITY, 0), ^{
                TVLog(@"AutoUnlock: starting boot unlock sequence...");

                // Step 1: Wait for SpringBoard to be fully up (up to 120s)
                BOOL sbReady = NO;
                for (int i = 0; i < 120; i++) {
                    int mib[4] = {CTL_KERN, KERN_PROC, KERN_PROC_ALL, 0};
                    size_t size = 0;
                    if (sysctl(mib, 4, NULL, &size, NULL, 0) == 0) {
                        ...
                        if (procs && sysctl(mib, 4, procs, &size, NULL, 0) == 0) {
                            ...
                            for (int j = 0; j < count; j++) {
                                if (strcmp(procs[j].kp_proc.p_comm, "SpringBoard") == 0) {
                                    sbReady = YES;
                                    break;
                                }
                            }
                ...
                if (!sbReady) {
                    TVLog(@"AutoUnlock: SpringBoard not found after 120s, aborting.");
                    return;
                }
                TVLog(@"AutoUnlock: SpringBoard detected, waiting 8s for lock screen UI to be ready...");

                // Step 2: Wait for lock screen UI to fully appear
                STAccurateSleep(8.0);

                // Step 3: Press Home to wake/raise lock screen
                dispatch_sync(dispatch_get_main_queue(), ^{
                    STHIDEventGenerator *gen = [STHIDEventGenerator sharedGenerator];
                    [gen menuPress];
                    TVLog(@"AutoUnlock: Home press #1 sent (wake screen)");
                });
                // Wait for lock screen animation to settle
                STAccurateSleep(1.5);

                // Step 4: Press Home again to dismiss lock screen (no-passcode unlock)
                dispatch_sync(dispatch_get_main_queue(), ^{
                    STHIDEventGenerator *gen = [STHIDEventGenerator sharedGenerator];
                    [gen menuPress];
                    TVLog(@"AutoUnlock: Home press #2 sent (dismiss lock screen)");
                });
                // Wait for unlock animation
                STAccurateSleep(0.8);

                // Step 5: Wait for "Touch ID" system alert to appear
                STAccurateSleep(2.0);

                // Step 6: Press Return to dismiss the "Unable to activate Touch ID" alert
                dispatch_sync(dispatch_get_main_queue(), ^{
                    STHIDEventGenerator *gen = [STHIDEventGenerator sharedGenerator];
                    [gen keyPress:@"\n"];
                    TVLog(@"AutoUnlock: Return key sent (dismiss Touch ID alert)");
                });
                STAccurateSleep(0.5);
